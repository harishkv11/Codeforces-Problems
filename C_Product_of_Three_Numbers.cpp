...
void solve(){
    ...
    for(int i=2;i*i<=n;i++){
        if(n%i==0) f.push_back({i,n/i});
    }
    
    for(auto x: f){
        if(!isprime(x.first)){
            for(int i=2;i*i<=x.first;i++){
                if(x.first%i==0 and i!=x.second and i!=x.first/i and x.second!=x.first/i){
                    cout<<"YES"<<endl;
                    cout<<i<<" "<<x.first/i<<" "<<x.second<<endl;
                    return;
                }
            }
        }
        else if(!isprime(x.second)){
            for(int i=2;i*i<=x.second;i++){
                if(x.second%i==0 and i!=x.second/i and i!=x.first and x.second/i!=x.first){
                    cout<<"YES"<<endl;
                    cout<<i<<" "<<x.second/i<<" "<<x.first<<endl;
                    return;
                }
            }
        }
